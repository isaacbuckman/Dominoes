...
#------------------------------------------------------------------------------------FOR ISMCTS-------------------------------------------------------------------------------------
    ...
    def clone_and_randomize(self):
        ...
        clone = self.clone()
        remaining_dominoes = all_dominoes()
        players = range(4)
        for t in clone.dominos_played:
        ...
        while curr_dominoes:
            ...
            curr_domino_probs = clone.probabilities[curr_domino]
            curr_assignment = choice(players, p=curr_domino_probs)
            clone._update_probs(curr_domino, curr_assignment)
        return clone

    #already implemented as "update(self, move, curr_player=None, placement=None)" 
    def do_move(self, move):
        """ update a state by carrying out the given move.
            Must update player_to_move.
        """
        self.update(move[0], placement=move[1])

    #already implemented as "possible_actions(self, curr_player=None, placements_included=True)"
    def get_moves(self):
